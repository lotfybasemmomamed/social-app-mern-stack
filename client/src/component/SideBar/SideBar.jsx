import { House } from "lucide-react";
import React from "react";

export default function SideBar() {
  const links = [
    {
      icon: <House className={` dark:text-white`} />,
      text: "feed",
    },
    { icon: <House className={` dark:text-white`} />, text: "feed" },
    { icon: <House className={` dark:text-white`} />, text: "feed" },
    { icon: <House className={` dark:text-white`} />, text: "feed" },
    { icon: <House className={` dark:text-white`} />, text: "feed" },
    { icon: <House className={` dark:text-white`} />, text: "feed" },
    { icon: <House className={` dark:text-white`} />, text: "feed" },
  ];
  const sidebarFooterLinks = [
    { text: "about", href: "#" },
    { text: "settings", href: "#" },
    { text: "support", href: "#" },
    { text: "docs", href: "#" },
    { text: "help", href: "#" },
    { text: "privacy & terms", href: "#" },
  ];
  return (
    <div>
      <div className="card-border bg-white dark:bg-gray-900">
        <div className="h-12.5 w-full   overflow-hidden">
          <img
            className="w-full h-full object-cover"
            src="https://img.freepik.com/free-photo/closeup-shot-beautiful-butterfly-with-interesting-textures-orange-petaled-flower_181624-7640.jpg?semt=ais_user_personalization&w=740&q=80"
          />
        </div>

        <div>
          <div className="w-18 h-18  border-3 relative left-1/2 -translate-x-1/2 -top-5 border-white rounded-sm">
            <img
              className="w-full h-full  object-cover"
              src="data:image/jpeg;base64,"
            />
          </div>

          {/* personal details */}
          <div>
            <h2 className="font-bold text-xl dark:text-white">Lotfy Basem</h2>
            <p className="text-gray text-[14px]">
              Frontend Developer at google
            </p>
          </div>
          <p className="text-gray mt-4 px-2  text-[14px]">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque
            perspiciatis id sed? Iure, illo ducimus!
          </p>
          {/* statictics */}

          <div className="flex justify-center mt-3 ">
            <div className="border-r pr-4 border-gray-400">
              <span className="font-bold text-[15px] dark:text-white">243</span>
              <p className="text-gray text-[14px]">Post</p>
            </div>
            <div className="border-r px-4 border-gray-400">
              <span className="font-bold text-[15px] dark:text-white">
                2.3K
              </span>
              <p className="text-gray text-[14px]">Followers</p>
            </div>
            <div className="pl-4">
              <span className="font-bold text-[15px] dark:text-white">365</span>
              <p className="text-gray text-[14px]">Following</p>
            </div>
          </div>
        </div>
        <div className="flex justify-center">
          <hr className="w-[80%] my-3 text-gray-200 dark:text-gray-400" />
        </div>
        {/* links */}
        <div>
          {links.map((link) => (
            <div className="flex gap-2 ml-7 ">
              <div className=" mb-3 flex gap-3 ">
                {link.icon}
                <p className="capitalize font-bold cursor-pointer hover:text-blue-600 dark:text-white dark:hover:text-blue-400">
                  {link.text}
                </p>
              </div>
            </div>
          ))}
        </div>
        <hr className="text-gray-200 dark:text-gray-400 my-3" />
        <p className="capitalize pb-3 text-[14px] text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-500 cursor-pointer">
          view profile
        </p>
      </div>
      <div className="flex justify-center flex-wrap gap-2 pt-6">
        {sidebarFooterLinks.map((link) => {
          return (
            <p className=" leading-tight cursor-pointer inline-block capitalize px-2 text-gray text-[14px] hover:text-blue-700! dark:hover:text-blue-500!">
              {link.text}
            </p>
          );
        })}
      </div>
      <p className="leading-tight cursor-pointer pt-2 capitalize px-2 pb-6 text-gray text-[14px] hover:text-blue-700! dark:hover:text-blue-500!">
        &copy; 2026
      </p>
    </div>
  );
}
